...
def Calculate():
  ...
  pet_fr = []
  bmi=""
  if request.method=="POST" and "size" in request.form and "Hydro" in request.form and "hometype" in request.form and "ACyesno" in request.form and "smoke_yesno" in request.form and "pet_yesno" in request.form and "parking" in request.form and "Bathrooms" in request.form and "wifi" in request.form and "Bedrooms" in request.form:
    hyd = np.zeros(4)
    ...
    list_all = smok + A_C + pet_fr + car_park + [sz] + z.tolist() + bath +type_home.tolist() + bed.tolist()+wifi_tv.tolist() +hyd.tolist()
    my_y = pd.DataFrame(list_all).T
    price = loaded_model.predict(my_y)
    price = str(price)[1:8]
